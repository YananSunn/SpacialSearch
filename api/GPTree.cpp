...
const char Data_File[]="./data/GPTree.data";
const char Car_File[]="./data/car.data";
...
struct Heap//双指针大根堆
{
    ...
    {
        printf("Heap:%d n=%d\n",this,n-1);
        printf("a:");for(int i=1;i<n;i++)printf(" %d",a[i]);cerr<<endl;
        printf("id:");for(int i=1;i<n-1;i++)printf(" %d",id[i]);cerr<<endl;
        printf("iid:");for(int i=1;i<n;i++)printf(" %d",iid[i]);cerr<<endl;
        printf("draw_end\n");
    }
};
...
struct Graph//无向图结构 
{
    ...
    {    
        ...
        for(int i=0;i<n;i++)cerr<<id[i]<<' ';cerr<<endl;
        for(int i=0;i<n;i++)
        {
            printf("%d:",i);
...
struct G_Tree
{
    ...
    struct Node
    {
        ...
        void save()
        {
            ...
            for(int i=0;i<part;i++)printf("%d ",son[i]);
            printf("\n");
            save_vector(color);
            dist.save();
            order.save();
        ...
        void write()
        {
            ...
            //G.draw();
            printf("color:");for(int i=0;i<(int)color.size();i++)printf("%d ",color[i]);cerr<<endl;
            printf("dist:\n");
            dist.write();
            printf("order:\n");
            order.write();
            printf("borders:");
            for(map<int,pair<int,int> >::iterator iter=borders.begin();iter!=borders.end();iter++)printf("(%d,%d,%d)",iter->first,iter->second.first,iter->second.second);cerr<<endl;
...
void addRoad(int a, int b, int c){
    Road* now = new Road;
    now->to = b, now->weight = c;
    now->next = roads[a], roads[a] = now;
}
...
void loadRoad(){
    ...
    int nodeSize, roadSize;
    fin >> nodeSize >> roadSize;

    // 初始化
    roads.resize(nodeSize);
...
sum: 当前总路径长度
route: 当前路径送客顺序
ans_route: 当前最优解的路径送客顺序
*/
void findBestPath(vector<int> & passenger, int step, vector<bool> & visited, 
    ...
    if (step == n){
        ...
        for (int i = 0; i < n; ++ i){
            ...
            if (!visited[i]){
                ...
                int x = passenger[i];
                route[step] = x;
                int dis = tree.search_catch(last, x);                
                findBestPath(passenger, step + 1, visited, ans, x, sum + dis, route, ans_route);
                visited[i] = false;
            }
        }
    }
...
// 初始状态下每辆出租车将车上乘客全部送到目的地的最短路径
vector<int> d1;

// 第一维-每个路口编号
// 第二维-路口的出租车编号
vector<vector<int>> taxiAtNode;

// 计算初始状态下每辆出租车将车上乘客全部送到目的地的最短路径 并写入文件
void saveCars(){
    ofstream fout(Car_File);
    for (int i = 0; i < passenger.size(); ++ i){
        vector<bool> v(5, false);
        int res = 1000000000;
        vector<int> tmp(passenger[i].size(), 0);
        findBestPath(passenger[i], 0, v, res, taxi[i], 0, tmp, tmp);
        fout << res << endl;
    }
    fout.close();
}
...
void loadCars(){
    ...
    while (fin >> id) {
        ...
        taxi.push_back(k);
        taxiAtNode[taxi[id]].push_back(id);
...
/*
searchTaxi
使用宽度优先搜索查询周围符合条件的出租车

src: 待查询乘客的出发点
dst: 待查询乘客的目的地
threshold: 查询数量
*/
vector<json> searchTaxi(int src, int dst, int threshold){
    vector<json> ans;
    ans.clear();

    // 待接乘客位置到自己目的地的最短路距离为D4
    int d4 = tree.search(src, dst);
    ...
    // 依次送达目的地所需路程为D1
    // 出租车接乘客上车所需路程为D2
    // 接到乘客后将所有k+1个乘客送达目的地所需路程为D3
    while (!nodeQueue.empty()){
        int x = nodeQueue.front();
        nodeQueue.pop();
        int d2 = tree.search(x, src);
        for (int j = 0; j < taxiAtNode[x].size(); j++){
            int i = taxiAtNode[x][j];
            int d3 = 1000000000;
            vector<bool> vis(5, false);

            // 将待查询乘客的目的地点加入当前假定的出租车的目的列表中
            vector<int> now_pass = passenger[i];
            now_pass.push_back(dst);

            vector<int> tmp(now_pass.size(), 0);
            vector<int> result(now_pass.size(), 0);
            findBestPath(now_pass, 0, vis, d3, src, 0, tmp, result);
            
            // 将出租车当前位置和待查询乘客出发位置加入结果列表
            result.insert(result.begin(), src);
            result.insert(result.begin(), x);

            // 通过两个绕路条件判断当前出租车是否符合要求
        ...
        // 更新下一个宽度优先搜索的路口
        for (Road *e = roads[x]; e != nullptr && e->weight + d2 <= 10000; e = e->next){
            // d2 + 当前位置到出发点的距离 剪枝
            if (!visited[e->to]) {
                visited[e->to] = true;
                nodeQueue.push(e->to);
...
int main(int argc, char **argv){
    if (argc == 2){
        init();
        read();
        Additional_Memory=4*G.n*log2(G.n);
        tree.build();
        save();
        cerr << "Begin saving car info\n";
        loadRoad();
        loadCars();
        saveCars();
        cerr << "Save successfully!\n";
        return 0;
    }
    vector<int> ans;
    load();
    cerr << "GPTree loaded!" << endl;
    loadRoad();
    cerr << "Roads loaded!" << endl;
    loadCars();
    loadD1();
    cerr << "Taxies loaded!" << endl;
    while (true){
        int src, dst;
        cin >> src >> dst;
        json result;
        auto ans = searchTaxi(src, dst, 5);
        if (ans.size() > 0)
